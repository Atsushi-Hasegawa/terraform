...
## 5. セキュリティ & ガードレール (Shift-Left Security)

本プロジェクトでは、インフラのデプロイ前に脆弱性を自動検知する「継続的セキュリティスキャン」を導入しています。

### セキュリティポリシーの 7 つの柱
1. **基礎・暗号化**: すべてのデータストア（RDS, S3, EBS）の暗号化と EC2 IMDS v2 の強制。
2. **最小権限・権限昇格防止**: IAM の `Action: "*"` 禁止と危険な特権の組み合わせ検知。
3. **横展開・ランサムウェア対策**: セキュリティグループの全開放禁止、S3 Object Lock と RDS 削除保護の強制。
4. **ガバナンス・統制**: 必須タグ（Project, Environment）、リージョン制限（東京のみ）、ログ出力の必須化。
5. **境界防御**: CloudFront/ALB への WAF 強制、管理ポート（SSH/RDP）のインターネット露出禁止。
6. **サプライチェーン**: Provider/Module のバージョン・ソース固定、シークレットのベタ書き検知。
7. **ライフサイクル**: KMS 削除待機期間の確保、非推奨エンジンバージョンの排除、自動パッチ適用の強制。

### ツール構成
- **Trivy**: 静的解析ツールとして、AWS/GCP の設定不備を自動検知。
- **OPA (Open Policy Agent)**: `policy/` 配下の Rego ファイルにより、組織独自のガードレールを適用。

---

## 6. 運用上の注意 (Git Management)
